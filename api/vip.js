...
/* ── SAME model as your existing API → existing codes stay valid ── */
const vipCodeSchema = new mongoose.Schema({
  ...
  /* NEW (optional — old docs simply won't have these) */
  redeemedBy: String,
  redeemedAt: Number,
  revokedAt: Number
});
...
export default async function handler(req, res){
  ...
  try{
    if(req.method === 'GET'){
      ...
      // Validate a specific code (used by the frontend VIP gate / expiry poll)
      if(query.check){
      ...
      // NEW: quick stats for the admin VIP tab
      if(query.type === 'stats'){
        const [total, active, used, revoked] = await Promise.all([
          VipCode.countDocuments({}),
          VipCode.countDocuments({ status: 'active' }),
          VipCode.countDocuments({ status: 'used' }),
          VipCode.countDocuments({ status: 'revoked' })
        ]);
        return res.status(200).json({ total, active, used, revoked });
      }

      // List all codes + redemptions (admin panel) — same shape as before
      const codes = await VipCode.find({}).sort({ created: -1 }).limit(300).lean();
    ...
    if(req.method === 'POST'){
      ...
      if(action === 'redeem'){
        ...
        /* NEW: track who redeemed (frontend passes user: userLabel()) */
        found.redeemedBy = user || 'Guest';
    ...
    if(req.method === 'PUT'){
      ...
      if(action === 'revoke'){
        await VipCode.findOneAndUpdate({ code }, { status: 'revoked', revokedAt: Date.now() });
        return res.status(200).json({ success: true });
      }
      if(action === 'revoke-all'){
        await VipCode.updateMany({ status: 'active' }, { status: 'revoked', revokedAt: Date.now() });
        return res.status(200).json({ success: true });
      }
      return res.status(400).json({ error: 'Unknown action' });
    }
