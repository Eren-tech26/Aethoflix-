...
export class ProviderSurface {
  ...
  load(media: EmbedMedia) {
    ...
    // Only Nxsha is sandboxed (per policy); all other providers load unrestricted.
    if (media.selection.server === 'nxsha') frame.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-presentation');
