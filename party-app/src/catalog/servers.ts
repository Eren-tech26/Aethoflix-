import type { CatalogTitle, EmbedMedia, EmbedSelection, ServerKey, ServerPreferences } from './types';
...
export const STD_SERVERS = [
  ...
  { key: 'nxsha', name: 'Nxsha', tag: 'HD + Subs', movie: (id: number) => `https://nxsha.space/embed/movie/${id}`, tv: (id: number, s: number, e: number) => `https://nxsha.space/embed/tv/${id}/${s}/${e}`, highlight: true },
...
export function serversFor(anime: boolean): { key: ServerKey; name: string; tag: string; number: number }[] {
  return (anime ? [...ANIME_SERVERS, ...STD_SERVERS] : STD_SERVERS).map((server, index) => ({ key: server.key, name: server.name, tag: server.tag, number: index + 1 }));
}
...
export function readServerPreferences(key: ServerKey): ServerPreferences {
  try {
    ...
    return { audio: value?.audio === 'dub' ? 'dub' : 'sub', source: value?.source === 'hd-2' ? 'hd-2' : RC_DEFAULT_SOURCE,
      ...
      sandbox: typeof value?.sandbox === 'boolean' ? value.sandbox : key === 'nxsha' };
...
export function makeEmbed(title: CatalogTitle, selection: EmbedSelection): EmbedMedia {
  ...
  if (standard) url = title.mediaType === 'movie' ? standard.movie(title.id) : standard.tv(title.id, selection.season, selection.episode);
    ...
    if (!preferences.zokoTemplate.trim()) throw new Error('Paste a Zokoanime embed URL or URL template. Its public route documentation could not be verified, so this app does not guess a playback URL.');
    let template = preferences.zokoTemplate.trim();
    if (template.includes('{anilistId}') && (!Number.isInteger(selection.animeId) || !selection.animeId || selection.animeId < 1)) throw new Error('Match the anime edition or enter its AniList ID first.');
    template = template.split('{anilistId}').join(String(selection.animeId ?? '')).split('{episode}').join(String(selection.animeEpisode)).split('{audio}').join(preferences.audio);
    if (/[{}]/.test(template)) throw new Error('The URL template contains an unknown field. Use only {anilistId}, {episode}, and {audio}.');
    let parsed: URL;
    try { parsed = new URL(template); } catch { throw new Error('Enter a complete HTTPS Zokoanime embed URL.'); }
    if (parsed.origin !== ZK_BASE || parsed.username || parsed.password || parsed.pathname === '/') throw new Error('The embed must be an HTTPS playback URL on zokoanime.video, not its homepage.');
    parsed.searchParams.set('color', ZK_COLOR.slice(7));
    url = parsed.toString();
  } else {
    ...
    url = selection.server === 'megaplay'
      ? `${MP_BASE}/stream/ani/${selection.animeId}/${selection.animeEpisode}/${preferences.audio}`
      : `${RC_BASE}/embed/${preferences.source}/ani/${selection.animeId}/${selection.animeEpisode}/${preferences.audio}?k=1`;
  }
