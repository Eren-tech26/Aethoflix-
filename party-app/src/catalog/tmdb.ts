...
type RawTitle = {
  ...
  overview?: string; poster_path?: string | null; backdrop_path?: string | null; release_date?: string; first_air_date?: string;
...
export async function getTitleDetails(title: CatalogTitle, endpoint: TmdbEndpoint, signal: AbortSignal): Promise<TitleDetails> {
  const raw = await request<RawTitle>(`/${title.mediaType}/${title.id}`, {}, endpoint, signal);
  return { ...normalize(raw, title.mediaType), anime: title.anime || normalize(raw, title.mediaType).anime,
    genres: raw.genres?.map((genre) => genre.name) ?? [], runtime: raw.runtime ?? raw.episode_run_time?.[0] ?? null,
...
export async function searchAnimeEditions(query: string, signal: AbortSignal): Promise<AnimeEdition[]> {
  ...
  try {
    const response = await fetch('https://graphql.anilist.co', {
      ...
      body: JSON.stringify({ query: 'query ($search: String) { Page(page: 1, perPage: 8) { media(search: $search, type: ANIME, isAdult: false) { id title { english romaji } startDate { year } format episodes coverImage { medium } } } }', variables: { search: query.trim() } }),
    });
    if (response.status === 429) throw new Error('AniList is rate-limiting requests. Wait a moment or enter the AniList ID manually.');
    if (!response.ok) throw new Error('AniList could not be reached. You can enter a verified AniList ID instead.');
    const data = await response.json() as { data?: { Page: { media: { id: number; title: { english: string | null; romaji: string }; startDate: { year: number | null }; format: string; episodes: number | null; coverImage: { medium: string | null } }[] } }; errors?: unknown[] };
    if (!data.data?.Page || data.errors) throw new Error('AniList returned no readable results. Try another title or enter an ID.');
    return data.data.Page.media.map((anime) => ({ id: anime.id, title: anime.title.english ?? anime.title.romaji, year: anime.startDate.year, format: anime.format, episodes: anime.episodes, poster: anime.coverImage.medium }));
  } catch (error) {
    if (signal.aborted) throw new DOMException('Cancelled', 'AbortError');
    if (controller.signal.aborted) throw new Error('AniList timed out. Enter an AniList ID manually or retry.');
    if (error instanceof TypeError) throw new Error('Cannot reach AniList. You can still enter an AniList ID manually.');
    throw error;
  } finally { clearTimeout(timeout); signal.removeEventListener('abort', cancel); }
}