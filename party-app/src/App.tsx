...
import { Armchair, ArrowRight, BellRing, Check, ChevronDown, CircleHelp, Film, Grid2X2, LoaderCircle, Maximize, Minimize, Monitor, Moon, Mouse, Pause, PersonStanding, Play, RotateCcw, Sun, Users, Volume2, VolumeX, X } from 'lucide-react';
...
export default function App() {
  ...
  const configuration = useRef({ quality, reducedMotion, panel, profile });
  configuration.current = { quality, reducedMotion, panel, profile };
  ...
  let actionLabel = snapshot.nearbySeatId && !snapshot.overview ? `Sit in ${snapshot.nearbySeatId}` : 'Take a seat';
  if (snapshot.mode === 'walking') actionLabel = `Walking to ${snapshot.seatId}`;
  if (snapshot.mode === 'sitting') actionLabel = 'Settling in...';
  if (snapshot.mode === 'seated') actionLabel = 'Stand up';
  if (snapshot.mode === 'standing') actionLabel = 'Standing up...';
  if (snapshot.reservingSeat) actionLabel = `Checking ${snapshot.reservingSeat}...`;

  return <>
    <div className={`app-shell ${ready ? 'is-ready' : ''} ${movieMode ? 'movie-session' : ''} ${reducedMotion ? 'reduced-motion' : ''}`} inert={panel ? true : undefined}>
      <header className="site-header">
        ...
        <nav className="main-nav" aria-label="Experience navigation"><button className="nav-link is-active" onClick={closePanel}>The cinema</button><button className="nav-link" onClick={() => setPanel('experience')}>The experience</button><button className="nav-link screen-nav-link" onClick={() => setPanel('screen')} disabled={!ready || !!error}>Screen player</button></nav>
        <div className="header-right"><button className="help-button" onClick={() => setPanel('controls')} aria-label="How to explore" title="How to explore"><CircleHelp size={17} strokeWidth={1.5} /></button><button className={`watch-party-button ${inParty ? 'is-connected' : ''}`} onClick={() => setPanel('party')} title="Watch Party: TMDB collection and shared screening"><Users size={15} strokeWidth={1.5} /><span>Watch party</span>{inParty && <span className="party-button-count">{watchParty.state.members.length}/10</span>}</button><button className="profile-button" onClick={() => setPanel('player')} aria-label={`Customize your player, ${profile.name}`} title="Your player"><PlayerAvatar profile={profile} /></button></div>
      </header>

      <main className="cinema-stage" aria-label="The AethoFlix private cinema">
        <CinemaScene onEngine={onEngine} onUpdate={onSnapshot} onReady={() => setReady(true)} onMessage={notify} onError={setError} onValidation={setValidation} />
        ...
        <div className={`room-mode ${movieMode ? 'is-cinema' : ''}`} aria-live="polite"><span className="mode-icon">{movieMode ? <Moon size={17} strokeWidth={1.3} /> : <Sun size={18} strokeWidth={1.3} />}</span><div><span className="mode-title">{movieMode ? 'CINEMA MODE' : snapshot.mode === 'walking' ? 'FINDING YOUR SEAT' : 'EXPLORATION MODE'}</span><span className="mode-description">{movieMode ? snapshot.lightLevel > 0.05 ? 'The lights are gently fading.' : 'Lights down. Story on.' : snapshot.mode === 'walking' ? `On your way to seat ${snapshot.seatId}.` : 'The room is yours.'}</span></div></div>
        <div className="scene-controls">
          ...
          <div className="seat-action"><div className="seat-action-buttons">{(movieMode || snapshot.eatingProgress !== null) && <button className={`service-bell-button ${snapshot.eatingProgress !== null ? 'is-eating' : ''}`} onClick={() => engine.current?.ringBell()} disabled={!ready || !!error || snapshot.servicePhase !== 'idle' || snapshot.eatingProgress !== null} title="Ring the bell for service" aria-label={snapshot.eatingProgress !== null ? 'Enjoying your order' : 'Ring for service'}>{snapshot.eatingProgress !== null ? <LoaderCircle className="spin" size={16} /> : <BellRing size={17} strokeWidth={1.5} />}<span>{snapshot.eatingProgress !== null ? `${snapshot.foodName} · ${Math.round(snapshot.eatingProgress * 100)}%` : snapshot.servicePhase !== 'idle' ? 'Server on the way' : 'Ring for service'}</span></button>}<button className={`primary-button take-seat-button ${snapshot.mode === 'seated' ? 'stand-button' : ''}`} onClick={primaryAction} disabled={!ready || !!error || transitioning || !!snapshot.reservingSeat} title={snapshot.mode === 'walking' ? 'Click to cancel the guided walk' : undefined}>{transitioning || snapshot.mode === 'walking' || snapshot.reservingSeat ? <LoaderCircle className="spin" size={18} /> : snapshot.mode === 'seated' ? <PersonStanding size={19} /> : <Armchair size={18} strokeWidth={1.65} />}<span>{actionLabel}</span>{snapshot.mode === 'walking' ? <X size={15} /> : snapshot.mode === 'seated' ? <kbd>E</kbd> : !transitioning && <ArrowRight size={17} />}</button><button className="seat-map-button" onClick={() => setPanel('seats')} disabled={!ready || !!error} aria-label="Choose a seat on the seating map" title="Choose your seat"><Grid2X2 size={18} strokeWidth={1.5} /></button></div>{snapshot.eatingProgress !== null && <span className="service-progress" aria-hidden="true"><i style={{ width: `${Math.round(snapshot.eatingProgress * 100)}%` }} /></span>}<span className="seat-action-caption">{movieMode ? `Seat ${snapshot.seatId}. Enjoy the view.` : snapshot.mode === 'walking' ? 'Use the movement controls to take over.' : snapshot.mode === 'standing' ? 'The room is yours to explore again.' : 'Make yourself comfortable.'}</span></div>
