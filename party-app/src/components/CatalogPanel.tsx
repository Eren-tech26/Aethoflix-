...
type Props = { snapshot: CinemaSnapshot; engine: CinemaEngine | null; party: WatchPartyController; onFinish: () => void; onSeats: () => void };

export default function CatalogPanel({ snapshot, engine, party, onFinish, onSeats }: Props) {
  ...
  return <>
    ...
    {!selected ? <>
      ...
      <div className="catalog-toolbar"><div className="catalog-tabs" role="group" aria-label="Content category">{([{ key: 'all', label: 'All titles' }, { key: 'movie', label: 'Movies' }, { key: 'tv', label: 'Series' }, { key: 'anime', label: 'Anime' }] as const).map((item) => <button key={item.key} aria-pressed={kind === item.key} onClick={() => { setKind(item.key); setPage(1); }}>{item.label}</button>)}</div><span>{query ? 'SEARCH RESULTS' : 'POPULAR RIGHT NOW'}</span></div>
      {busy ? <div className="catalog-loading" role="status"><LoaderCircle className="spin" size={28} /><span>{query ? 'Finding your stories...' : 'Opening the collection...'}</span></div> : error ? <div className="catalog-error" role="alert"><Globe2 size={30} /><h3>Let us try another route.</h3><p>{error}</p><div><button className="secondary-button" onClick={() => setAttempt((value) => value + 1)}><RotateCcw size={15} />Retry</button><button className="primary-button" onClick={() => changeEndpoint(endpoint === 'standard' ? 'alternate' : 'standard')}>Try {endpoint === 'standard' ? 'alternate' : 'standard'} endpoint<ArrowRight size={16} /></button></div></div> : results?.results.length ? <div className="catalog-grid">{results.results.map((title) => <button key={`${title.mediaType}-${title.id}`} className="catalog-result" onClick={() => setSelected(title)}><Poster title={title} /><span className="catalog-result-title">{title.title}</span><span className="catalog-result-meta"><span>{title.year || 'Year TBA'}<i />{title.anime ? 'Anime' : title.mediaType === 'movie' ? 'Movie' : 'Series'}</span>{title.rating > 0 && <span><Star size={11} />{title.rating.toFixed(1)}</span>}</span></button>)}</div> : <div className="catalog-empty"><Search size={30} /><h3>No stories found just yet.</h3><p>Try another title or category.{kind === 'anime' ? ' Anime results use TMDB Japanese-animation metadata.' : ''}</p></div>}
    ...
    </> : <TitleView key={`${selected.mediaType}-${selected.id}`} title={selected} endpoint={endpoint} snapshot={snapshot} engine={engine} party={party} canControl={canControl} onBack={() => setSelected(null)} onSeats={onSeats} onFinish={onFinish} />}
...
function TitleView({ title, endpoint, snapshot, engine, party, canControl, onBack, onSeats, onFinish }: { title: CatalogTitle; endpoint: TmdbEndpoint; canControl: boolean; onBack: () => void } & Omit<Props, 'onFinish'> & { onFinish: () => void }) {
