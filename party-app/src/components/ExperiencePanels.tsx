...
function AboutPanel({ onClose }: { onClose: () => void }) {
  return <>
    ...
    <div className="about-note"><Compass size={19} /><p>Create a local Watch Party to browse the TMDB collection, pick a server, and share the screen, players, and seating between tabs in the same browser. Cross-device rooms are not connected yet.</p></div>
...
const QUALITY_OPTIONS: { value: Quality; label: string; description: string }[] = [
  { value: 'high', label: 'Ultra graphics', description: 'Maximum detail and high-DPI rendering. Best on strong GPUs.' },
  { value: 'auto', label: 'Adaptive', description: 'Targets your display: up to 120 fps on 120 Hz screens, adjusting resolution only when needed.' },
  { value: 'performance', label: 'Performance', description: 'Lightest rendering for older devices and battery saver mode.' },
];

function SettingsPanel({ quality, onQuality, reducedMotion, onReducedMotion, snapshot, validation, onReset }: {
  ...
  return <>
    ...
    <div className="settings-section"><div className="section-label"><Monitor size={15} /><span>RENDER QUALITY</span><span className="actual-fps">{snapshot.fps || '--'} FPS</span></div>
      <fieldset className="quality-options"><legend className="sr-only">Render quality</legend>{QUALITY_OPTIONS.map((option) => <label className="quality-option" key={option.value}>
        ...
        {option.value === 'high' && <span className="recommended-label">RECOMMENDED</span>}
      </label>)}</fieldset>
...
export default function ExperiencePanels(props: Props) {
  ...
  return <Dialog kind={props.panel} onClose={props.panel === 'catalog' ? props.onCloseCatalog : props.onClose}>
    ...
    {props.panel === 'catalog' && <CatalogPanel engine={props.engine} snapshot={props.snapshot} party={props.party} onFinish={props.onClose} onSeats={() => props.onOpenPanel('seats')} />}
